...
class TankGame(object):
    ...
    def __init__(self):
        from lib.Const import CREATE_ENEMY_EVENT
    ...
    def __init_sprites(self):
        ...
        from lib.Tanks import PlayerTank
        # 游戏开始界面
        self.menu = Menu()
    ...
    def __startMenu(self):
        from lib.Const import screen
        from lib.Const import SCREEN_RECT
        # 初始化
        screen.fill((0, 0, 0))
        self.level = 0
        # 游戏开始界面
    ...
    # 画出所有图片
    def __drawAll(self, username):
        from lib.Const import tanks_group, map_Group, bullet_group, props_group, screen
        # 设置游戏背景
        ...
        if self.player1_tank.lives > 0:
            ...
            surface_1 = font.render("用户：", True, (0, 0, 0), None)  # 透明
            screen.blit(surface_1, (630, 470))
            surface_2 = font.render(f"{username}", True, (0, 0, 0), None)  # 透明
            screen.blit(surface_2, (630, 490))
    ...
    def __check_collision(self):
        from lib.Const import bang, bullet_group, props_group, add, props
        # 碰撞
        for enemyTank in self.enemyTank_group:
                    ...
                    if not enemyTank.isDestroyed:
                        if pygame.sprite.collide_rect(playerTank.bullet, enemyTank):
                            ...
                            if enemyTank.lives < 0:
                                ...
                                if playerTank.player == 1:
                                    self.p1_des_num += 1
                                    print("player 1 destory num:", self.p1_des_num)
                                else:
                                    self.p2_des_num += 1
                                    print("player 2 destory num:", self.p2_des_num)
                                enemyTank.destroy()
    ...
    def __level(self, l=1):
        from lib.Const import map_Group, tanks_group, bullet_group, props_group, GAME_STATE
        from lib.Tanks import EnemyTank
        self.level += l
    ...
    # 游戏结束
    def __game_over(self, username):
        from lib.Const import screen, GAME_STATE, SCREEN_HEIGHT
        screen.fill((0, 0, 0))
        # 图片
        ...
        if self.overY < SCREEN_HEIGHT // 2 - 150:
            ...
            self.overY = 640
        self.save_game_history(username)

    # 游戏状态监听
    def _state_handler(self, username):
        from lib.Const import GAME_STATE, map_Group
        if self.gameState == GAME_STATE['GAME_MENU']:
            ...
            if not self.map.isReady:
                ...
                self.save_flag = 0  # 每次初始化让flag=0 确保只保存一次
                self.gameState = GAME_STATE['GAME_START']
        elif self.gameState == GAME_STATE['GAME_START']:
            # 开始游戏
            try:
                self.__drawAll(username=username)
            except Exception as e:
                print(f'{e}')
            # 游戏结束
            if (not map_Group.has(self.map.home)) or (self.player1_tank.lives <= 0 and self.player2_tank.lives <= 0):
    ...
    def __event_handler(self):
        from lib.Const import GAME_STATE, CREATE_ENEMY_EVENT, tanks_group
        from lib.Tanks import EnemyTank
    ...
    def __KeyDown_handler(self):
        from lib.Const import GAME_STATE, UP, DOWN, LEFT, RIGHT
    ...
    def start_game(self, username):
        from lib.Const import FRAME_PER_SEC
        while True:
            # FPS
            ...
            # 游戏状态监听
            self._state_handler(username)
            # 碰撞检测
            self.__check_collision()
            # 更新显示
            pygame.display.update()

    def save_game_history(self, username):
        if self.save_flag == 0:
            self.save_flag = 1
            type = None
            ...
            if self.player2_tank.lives > 0:
                ...
                type = '单人'
            print("玩家一击落", self.p1_des_num, " 玩家二击落 ", self.p2_des_num)
            cnx = pymysql.connect(user='root', password='123456',
                                  host='localhost', database='tank')
            cursor = cnx.cursor()
            query = ("INSERT INTO history (username, time,type,user_1_defeat,user_2_defeat) VALUES (%s, %s,%s, %s,%s)")
            cursor.execute(query, (username, time, type, self.p1_des_num, self.p2_des_num))
            self.p1_des_num = 0
            self.p2_des_num = 0
            cnx.commit()
